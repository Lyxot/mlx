  ...
  if constexpr (k_major.value) {
    if (has_k_residue) {
      throw std::invalid_argument(
          fmt::format("{} K must be multiples of max(64, group_size).", tag));
    }
...
void qmm_naive(
  ...
  dispatch_bool(transpose, [&](auto k_major) {
    dispatch_k_residue(
        k_major, has_k_residue, "[quantized_matmul]", [&]<bool HasKResidue>() {
...
void gather_qmm_naive_rhs(
    ...
    cu::CommandEncoder& encoder) {
  auto dispatch = [&]<int TileM, bool KMajor, bool HasKResidue, bool SM80>() {
    gather_qmm_naive_rhs_impl<TileM, KMajor, HasKResidue, SM80>(
        x,
        w,
        scales,
        biases,
  ...
  dispatch_bool(transpose, [&](auto k_major) {
    dispatch_k_residue(
        k_major, has_k_residue, "[gather_qmm]", [&]<bool HasKResidue>() {
          dispatch_bool(sm80, [&](auto sm80) {
            constexpr bool KMajor = k_major.value;
            constexpr bool SM80 = sm80.value;
            if (m <= 16) {
